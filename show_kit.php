...
function show_kit($salesman_id,$email,$conn)
{
    ...
    while($row = mysql_fetch_assoc($kit))
		...
		$product = color($product,$row['product_sku'],$conn);
		$product = image($product,$row['product_sku'],$conn);
		$product = price($salesman_id,$email,$product,$row['product_sku'],$conn);
		$product = description($product,$row['product_sku'],$conn);
		$product = product_name($product,$row['product_sku'],$conn);
		$product = product_sku($product,$row['product_sku'],$conn);
                $product = product_fit($product,$row['product_sku'],$conn);
                $data['elements'][] = $product;
                unset($product);
		
        }
        echo json_encode($data);
    }
	function size($buscket,$product_sku,$conn)
{
		$q0 = mysql_fetch_assoc(mysql_query("SELECT `size` FROM `product_item` WHERE `product_sku` = '$product_sku'",$conn));
		$res2 = mysql_fetch_assoc(mysql_query("SELECT `size` FROM `product_sub_category` WHERE `sub_category_id` = '".$q0['sub_category_id']."'",$conn));
		$sizes = explode(",", $res2['size']);
		$buscket['size'] =  $q0['size'];
		return $buscket;
}
function color($buscket,$product_sku,$conn)
{
	...
	$Q1 = "SELECT * FROM `product_item` WHERE `parent_product_sku` = '$product_sku'";
	$color2 = mysql_query($Q1,$conn);
	while($r1 = mysql_fetch_assoc($color2))
	{
	$buscket['color'][] = $r1['color'];
	}
	return $buscket;
}
function image($buscket,$product_sku,$conn)
{
...
function price($salesman_id,$email,$buscket,$product_sku,$conn)
{
...
function description($buscket,$product_sku,$conn)
{
...
function product_name($buscket,$product_sku,$conn)
{
...
function product_sku($buscket,$product_sku)
{
$buscket['product_sku'] =  $product_sku;
return $buscket;
}
function product_fit($buscket,$product_sku,$conn)
{
      $q7 = "SELECT `fit` FROM `product_item` WHERE `product_sku` = '$product_sku'";
      $fit = mysql_query($q7,$conn);
      while($row = mysql_fetch_assoc($fit))
      {
      $buscket['fit'] = $row['fit'];
      }
       return $buscket;
